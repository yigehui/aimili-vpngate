...
POOL_HEALTH_LOOP_ENABLED = env_bool("POOL_HEALTH_LOOP_ENABLED", True)
POOL_HEALTH_CHECK_INTERVAL_SECONDS = env_int("POOL_HEALTH_CHECK_INTERVAL_SECONDS", 60, 5)
...
def test_multiple_nodes(node_ids: list[str]) -> list[dict[str, Any]]:
    ...
    if available_snapshot is not None:
        try:
            pool_manager.replace_pool_from_nodes(available_snapshot)
        except Exception as pool_exc:
            print(f"[test_multiple_nodes] pool replace failed: {pool_exc}", flush=True)
...
def build_pool_manager() -> proxy_pool.PoolManager:
    ...
    mgr = proxy_pool.PoolManager(
        ...
        max_shadow_starting=int(cfg.get("max_shadow_starting", 0)),
